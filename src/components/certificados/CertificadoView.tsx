...
export function CertificadoView({ muestra, proyecto, norma, onBack }: CertificadoViewProps) {
  ...
  const validationUrl = `https://construlab-demo.com/verify/${muestra.id}`;
  const digitalSignature = btoa(`${muestra.id}-${muestra.fechaRecepcion}-${muestra.estado}`).substring(0, 32);
  const primaryColor = activeTemplate.primaryColor || '#000000';

  // --- SUB-COMPONENTS FOR CLEANER RENDER ---

  const Header = () => {
    if (activeTemplate.layout === 'modern') {
      return (
  ...
  const SampleInfo = () => {
    const isModern = activeTemplate.layout === 'modern';
    
    return (
      <div className={cn(
        "mb-8 text-sm",
        isModern ? "bg-gray-50 p-6 rounded-xl" : "bg-white border p-4",
        activeTemplate.layout === 'minimal' && "border-none bg-transparent p-0 grid grid-cols-2 gap-8"
      )} style={{ borderColor: isModern ? 'transparent' : '#e5e7eb' }}>
        
        {activeTemplate.layout !== 'minimal' && (
           <h3 className="font-bold border-b border-gray-200 pb-2 mb-4 uppercase text-xs text-gray-500">
             Información de la Muestra
           </h3>
        )}

        <div className={cn("grid gap-4", activeTemplate.layout === 'minimal' ? "grid-cols-1" : "grid-cols-2 md:grid-cols-3")}>
            <div>
    ...
    <div className="mb-8">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className={cn(
            "border-b", 
            activeTemplate.layout === 'modern' ? "bg-gray-100 border-none" : "bg-gray-50 border-gray-300"
          )}>
  ...
  const Footer = () => (
    <div className="mt-auto pt-12">
      ...
      <div className={cn(
        ...
        activeTemplate.layout === 'modern' ? "bg-gray-900 text-white" : "bg-gray-50 border border-gray-200"
      )}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={cn(
        ...
        activeTemplate.showBorder && "border-2"
      )} style={{ borderColor: activeTemplate.showBorder ? primaryColor : 'transparent' }}>
